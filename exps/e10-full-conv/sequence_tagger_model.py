...
class SequenceTagger(flair.nn.Classifier):
    def __init__(
        ...
        if self.use_rnn:
            ...
            self.span_length = 3
            self.entity_proj = torch.nn.Sequential(
                torch.nn.Conv1d(hidden_size * num_directions, hidden_size, 7, 1, 3),
                torch.nn.BatchNorm1d(hidden_size),
                torch.nn.LeakyReLU(),
                torch.nn.Conv1d(hidden_size, hidden_size, 7, 1, 3),
                torch.nn.BatchNorm1d(hidden_size),
                torch.nn.LeakyReLU(),
                torch.nn.Conv1d(hidden_size, self.span_length*2 + 1, 1))
    ...
    def predict(
        ...
        with torch.no_grad():
            ...
            for batch in dataloader:
                ...
                feature, entity_sim = self.forward(batch)
    ...
    def forward_loss(
        ...
        features, entity_sim = self.forward(data_points)
        score, token_count = self._calculate_loss(features, data_points)
        return score + self._cal_entity_loss(entity_sim, data_points), token_count
    ...
    def forward(self, sentences: List[Sentence]):
        ...
        sentence_tensor_ = sentence_tensor.transpose(-1, -2)
        entity_sim = torch.nn.functional.sigmoid(self.entity_proj(sentence_tensor_).transpose(-1, -2))
        
        # print(entity_sim.shape)
        features = self.linear(sentence_tensor)

        return features, entity_sim
    ...
    def _cal_entity_loss(self, features, sentences: List[Sentence], weight=0.2):
        tag_list: List = []
        lengths: List[int] = [len(sentence.tokens) for sentence in sentences]
        
        for sentence in sentences:
            tag = torch.tensor(sentence.entity_relation, device=flair.device).float()
            tag_list.append(tag)
        ...
        for sentence_feats, sentence_tags, sentence_length in zip(
            ...
            # print(sentence_feats.shape, sentence_tags.shape)
            score += torch.nn.functional.mse_loss(
                sentence_feats, sentence_tags, reduction='mean',
            )
    ...
    def _calculate_loss(
        ...
        # for sentence in sentences:
        #     print(sentence.entity_relation.shape, len(sentence))
        lengths: List[int] = [len(sentence.tokens) for sentence in sentences]
