...
class SequenceTagger(flair.nn.Classifier):
    def __init__(
        ...
        self.with_reg_loss = with_reg_loss
        self.reg_map = [
            ('B', 'S'),
            ('I', 'S'),
            ('B', 'O'),
    ...
    def _reg_loss(self, features):
        N, C = features.shape
        features = F.softmax(features, dim=1)
        pad_values = features.new_zeros(1, C)
        pad_features = torch.cat([features, pad_values], dim=0)

        all_tags = self.tag_dictionary.get_items()[1:]
        labels1 = []
        labels2 = []
        for tag1 in all_tags:
            for tag2 in all_tags:
                if (tag1[0], tag2[0]) in self.reg_map:
                    # print(tag1[0], tag2[0])
                    lab1 = self.tag_dictionary.get_idx_for_item(tag1)
                    lab2 = self.tag_dictionary.get_idx_for_item(tag2)
                    labels1.append(lab1)
                    labels2.append(lab2)
                
        loss = (features[:, labels1] * pad_features[1:, labels2]).mean(dim=1).sum()
        # print(loss)
        return loss

    ## MODIFIED: penalty loss
    def _calculate_loss(
        ...
        if self.use_crf:
            ...
            reg_loss = 0.
            if self.with_reg_loss:
                for sentence_feats, sentence_tags, sentence_length in zip(
                        features, tag_list, lengths
                ):
                    sentence_feats = sentence_feats[:sentence_length]
                    reg_loss += 5*self._reg_loss(sentence_feats)
            ...
            return reg_loss + score.sum(), token_count
            ...
            for sentence_feats, sentence_tags, sentence_length in zip(
                ...
                if self.with_reg_loss:
                    score += 5*self._reg_loss(sentence_feats)
