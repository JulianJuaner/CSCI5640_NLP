...
class SequenceTagger(flair.nn.Classifier):
    def __init__(
        ...
        if self.use_rnn:
            ...
            ## MODIFIED: entity relationship branch
            self.span_length = 3
            self.entity_proj = torch.nn.Sequential(
                torch.nn.Linear(hidden_size * num_directions, hidden_size),
                torch.nn.LayerNorm(hidden_size),
                torch.nn.LeakyReLU(),
                torch.nn.Linear(hidden_size, self.span_length*2 + 1))
    ...
    ## MODIFIED: entity relationship loss
    def forward_loss(
    ...
    def forward(self, sentences: List[Sentence]):
        ...
        ## MODIFIED: entity relationship
        entity_sim = torch.nn.functional.sigmoid(self.entity_proj(sentence_tensor))
